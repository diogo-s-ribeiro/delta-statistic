...
def result(request):
    ...
    if doyou1 != 'Yes':
        ...
        arr_ancest = np.array(list(filter(lambda x: x != [] and x != [''], arr_ancest)))
        # Remove first row based on string content
        if bool(re.search(r'[a-df-zA-DF-Z]', arr_ancest[0][1])):
            arr_ancest = arr_ancest[1:]
        # Remove first column based on string content
        if bool(re.search(r'[a-df-zA-DF-Z]', arr_ancest[0][0])):
            arr_ancest = arr_ancest[0:, 1:]
        #Append
        marg = np.array(arr_ancest, dtype=float)
        astates += [marg]
